...
export function CanvasTimeline() {
  ...
  useEffect(() => {
    ...
    const handleDrop = async (e: DragEvent) => {
      ...
      // Handle asset drop (from assets panel or FJ media bin)
      const assetId = e.dataTransfer!.getData("application/x-asset-id");
      if (assetId) {
        let asset = d.assets.find((a) => a.id === assetId);

        // Not in store yet — try injecting from the FJ asset payload
        if (!asset) {
          try {
            const raw = e.dataTransfer!.getData("application/x-fj-asset");
            if (raw) {
              await injectFjAsset(JSON.parse(raw));
              asset = useAssetStore.getState().assets.find((a) => a.id === assetId);
            }
          } catch {
            // ignore
          }
