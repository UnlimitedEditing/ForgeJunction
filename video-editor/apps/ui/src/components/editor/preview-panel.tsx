...
export function PreviewPanel() {
  ...
  // Declared early so getOrCreateImageElement can schedule re-renders on image load
  ...
  const getOrCreateImageElement = useCallback((asset: MediaAsset): ImageEntry | null => {
    ...
    // CDN assets injected via FJ bridge have an empty placeholder file.
    // Use the URL directly; no object URL to revoke later.
    const isCdnAsset = asset.file.size === 0 && asset.url !== "";
    const objectUrl = isCdnAsset ? "" : URL.createObjectURL(asset.file);
    img.src = isCdnAsset ? asset.url : objectUrl;
    ...
    img.onload = () => {
      entry.isReady = true;
      // CDN images load asynchronously — schedule a re-render so the texture
      // gets uploaded now that the element is ready.
      requestAnimationFrame(() => {
        if (renderFrameRef.current && !useVideoEditorStore.getState().isPlaying) {
          void renderFrameRef.current(useVideoEditorStore.getState().currentTime);
        }
      });
    };
    ...
    async (time: number) => {
      ...
      try {
        ...
        for (const clip of visibleMediaClips) {
          ...
          // CDN assets (injected via FJ bridge) have an empty placeholder file — use URL directly.
          const videoSource: Blob | string = asset.file.size === 0 && asset.url ? asset.url : asset.file;

          try {
            const loader = await loaderManager.getLoader(loaderKey, videoSource);
