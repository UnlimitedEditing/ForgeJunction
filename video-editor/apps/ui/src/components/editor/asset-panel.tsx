...
import { injectFjAsset } from "../../fj-bridge";
...
export function AssetPanel() {
  ...
  const isFjAssetDrag = (types: readonly string[]) =>
    types.includes("application/x-fj-asset");

  const handleDragEnter = useCallback((e: DragEvent) => {
    e.preventDefault();
    // Show indicator for file drops or FJ asset drops
    if (e.dataTransfer.types.includes("Files") || isFjAssetDrag(e.dataTransfer.types)) {
  ...
  const handleDrop = useCallback(
    async (e: DragEvent) => {
      ...
      // FJ asset drop — inject into Tooscut stores
      if (e.dataTransfer.types.includes("application/x-fj-asset")) {
        try {
          const raw = e.dataTransfer.getData("application/x-fj-asset");
          if (raw) {
            await injectFjAsset(JSON.parse(raw));
            setActiveTab("assets");
          }
        } catch {
          // ignore parse errors
        ...
        <div className="relative flex-1 overflow-auto">
          ...
          {isDragOver && (
            <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/80 border-2 border-dashed border-primary m-2 rounded-lg pointer-events-none">
              <div className="text-sm font-medium text-primary">Drop to import</div>
            </div>
          )}
        </div>
      </div>
