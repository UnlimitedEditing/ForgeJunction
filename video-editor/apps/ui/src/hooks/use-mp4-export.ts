...
/**
 * Get a Blob for an asset. For locally-imported assets the File is returned directly.
 * For CDN assets injected via the FJ bridge (empty placeholder file), the URL is fetched.
 */
async function getAssetBlob(asset: { file: File; url: string }): Promise<Blob | null> {
  if (asset.file.size > 0) return asset.file;
  ...
  const startExport = useCallback(async (options: ExportOptions): Promise<ExportResult> => {
    ...
    try {
      ...
      for (const clip of mediaClips) {
        ...
        const videoBlob = await getAssetBlob(asset);
        if (!videoBlob) continue;
        loadedVideoAssets.add(asset.id);
        await pool.loadVideoAsset(asset.id, videoBlob);
      }
      ...
      if (audioClips.length > 0) {
        try {
          ...
          for (const clip of audioClips) {
            ...
            const audioBlob = await getAssetBlob(asset);
