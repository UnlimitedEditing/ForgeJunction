...
export default function BinNode({ node, isSelected, isSnapTarget, animationClass = '', onContextMenu }: Props): React.ReactElement {
    ...
    >
      ...
      >
        {items.length === 0 ? (
          ...
          >
            {items.map((item, i) => (
              <div key={i} className="relative aspect-square rounded-lg overflow-hidden bg-white/5 group cursor-pointer">
      ...
      <div
        className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full border z-10 transition-all ${
          isSnapTarget
            ? 'bg-emerald-400 border-emerald-300 scale-150'
      ...
      {/* Resize handle */}
      <div
        className="absolute bottom-0 right-0 w-5 h-5 cursor-se-resize opacity-20 hover:opacity-50 transition-opacity rounded-br-xl"
