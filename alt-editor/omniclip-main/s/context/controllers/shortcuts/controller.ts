...
export type ActionType =  "Undo" | "Redo" | "Paste" | "Copy" | "Delete" | "Split" | "Cut" | "Next frame" | "Previous frame" | "Play/Pause timeline" | "Add marker" | "Next marker" | "Previous marker"
interface Shortcut {
	type: string
	shortcut: string
...
export class Shortcuts {
	...
	#getDefaultActionForDescription(description: ActionType): ShortcutAction | null {
		switch (description) {
			...
			case "Add marker":
				return (state) => {
					this.actions.add_marker(state.timecode)
				}
			case "Next marker":
				return (state) => {
					const next = [...state.markers]
						.sort((a, b) => a.time - b.time)
						.find(m => m.time > state.timecode + 10)
					if (next) this.actions.set_timecode(next.time, {omit: true})
				}
			case "Previous marker":
				return (state) => {
					const prev = [...state.markers]
						.filter(m => m.time < state.timecode - 10)
						.sort((a, b) => b.time - a.time)[0]
					if (prev) this.actions.set_timecode(prev.time, {omit: true})
				}
...
export const DEFAULT_SHORTCUTS: {actionType: ActionType; type: string; shortcut: string}[] = [
	...
	{ actionType: "Add marker", type: "marker", shortcut: "m" },
	{ actionType: "Next marker", type: "marker", shortcut: "ctrl+m" },
	{ actionType: "Previous marker", type: "marker", shortcut: "shift+m" },
]
