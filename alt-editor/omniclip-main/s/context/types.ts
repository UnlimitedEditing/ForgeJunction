...
export interface FjMarker {
	id: string
	time: number   // milliseconds from timeline start
	label?: string
}

export interface FjTag {
	id: string
	name: string
	color: string
}

export interface FjTagAsset {
	id: string
	url: string
	name: string
...
export interface NonHistoricalState {
	...
	log: string
	settings: Settings
	/** Timeline markers (added with M, navigated with Ctrl+M / Shift+M) */
	markers: FjMarker[]
	/** FJ tags synced from parent window via fj:sync-tags */
	fj_tags: FjTag[]
	/** Ordered FJ assets per tag (tagId → assets sorted by tag index) */
	fj_tag_assets: Record<string, FjTagAsset[]>
}
