...
export const non_historical = actionize_non_historical({
	...
	// ── FJ Markers ────────────────────────────────────────────────────────────
	add_marker: state => (time: number, label?: string) => {
		const id = generate_id()
		state.markers.push({id, time, label})
		state.markers.sort((a, b) => a.time - b.time)
	},
	remove_marker: state => (markerId: string) => {
	...
	// ── FJ Tags sync (from parent postMessage) ─────────────────────────────────
	set_fj_tags: state => (tags: FjTag[], tagAssets: Record<string, FjTagAsset[]>) => {
		state.fj_tags = tags
		state.fj_tag_assets = tagAssets
	},
})
